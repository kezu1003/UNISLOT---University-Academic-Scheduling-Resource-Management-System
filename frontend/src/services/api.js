...
// ✅ FIX: Response interceptor - only logout on auth routes failing
api.interceptors.response.use(
  ...
  (error) => {
    ...
    // 1. Token is truly expired (/auth/me returns 401)
    // 2. NOT on login page already
    // 3. NOT a data API call (coordinator, admin, lic endpoints)
    if (
      status === 401 &&
      !isLoginPage &&
